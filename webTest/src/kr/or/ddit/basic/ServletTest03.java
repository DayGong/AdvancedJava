...
/*
 * - Servlet의 동작 과정
   1. 사용자(클라이언트)가 URL을 클릭하면 HTTP요청을 Servlet Container로 전송한다.(요청 작업)
   2. 컨테이너는 web.xml에 정의된 url패턴을 확인하여 어느 서블릿을 통해 처리해야 할지를 검색한다.
   	  (로딩이 안 된 경우에는 로딩을 한다. 처음 로딩시 init()메서드를 호출한다.)
   	  (서블릿 3.0이상에서는 애노테이션(@WebServlet)으로 설정한다.)
   3. 서블릿 컨테이너는 개별 요청을 처리할 쓰레드를 생성하여 해당 서블릿 객체의 service()메서드를 호출한다.
      (이때 HttpServletRewuest객체와 HttpServletResponse객체를 생성하여 파라미터로 넘겨준다.)
...
public class ServletTest03 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<body><h2 style='color:blue;'>doGet()메서드에서 처리한 내용입니다.</h2></body>");
		out.println("</html>");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html>");
		out.println("<head><meta charset='utf-8'></head>");
		out.println("<body><h2 style='color:red;'>doPost()메서드에서 처리한 내용입니다.</h2></body>");
		out.println("</html>");
	}
	
	@Override
	public void destroy() {
		System.out.println(this.getServletName() + "에서 destroy()메서드를 호출");
	}
